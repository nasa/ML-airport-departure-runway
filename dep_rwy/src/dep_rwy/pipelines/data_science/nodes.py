...
def _train_model(
    ...
    # Check for grid search instructions
    if "CV_params" in model_params:
        if "regressor" in dir(model):
            base_model = model
            base_model.regressor = type(model.regressor)()
        else:
...
def _log_model_to_mlflow(
    ...
    with mlflow.start_run(run_id=run_id):
        ...
        mlflow.log_param(
            "core_features",
            [n for n in inputs if inputs[n]["core"] == True],
            )

        if "baseline" in model_params:
            mlflow.log_param("baseline", model_params["baseline"])

        mlflow.log_param("start_time", global_params["start_time"])
        mlflow.log_param("end_time", global_params["end_time"])
        mlflow.log_param("known_runways", global_params["known_runways"])
        mlflow.log_param("default_response", model.default_response)

        mlflow.set_tag("airport_icao", global_params["airport_icao"])
        mlflow.set_tag("grid_search_run", "CV_params" in model_params)

        if "CV_params" in model_params:
            mlflow.log_param("CV_params", model_params["CV_params"])

            best_params = {
                re.sub(r"^regressor__", "", key):val
                for key, val in model.named_steps["model"].best_params_.items()
                }
            mlflow.log_param("best_model_params", best_params)

            grid_search_filename = "gs_results.csv"
            pd.DataFrame(model.named_steps["model"].cv_results_).to_csv(
                grid_search_filename,
                index=False,
                )

            f = Path(grid_search_filename)
            if f.exists():
                mlflow.log_artifact(grid_search_filename)
                f.unlink()
            else:
                log.error(f"Looked for {grid_search_filename} but could not find it locally")

        all_params = {
                re.sub(r"^regressor__", "", key):val
                for key, val in model_params["model_params"].items()
                }
        mlflow.log_param("model_params", all_params)
